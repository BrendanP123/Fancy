...
{
    public static boolean isNeighbouringBlock(Coord coord1, Coord coord2, World world)
    {
        int sides = 6;
        
        for(ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS)
        {
            int newX = (int) (coord1.x + direction.offsetX);
            int newY = (int) (coord1.y + direction.offsetY);
            int newZ = (int) (coord1.z + direction.offsetZ);
            
            if(coord2.x == newX || coord2.y == newY || coord2.z == newZ)
            {
                sides--;
            }
        }
    ...
    public static int getBlockMetaFromNeighbour(int x, int y, int z, ForgeDirection side, World world)
    {
        return world.getBlockMetadata(x + side.offsetX, y + side.offsetY, z + side.offsetZ);
    }
